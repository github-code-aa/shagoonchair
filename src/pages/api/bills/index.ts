...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const missingFields = requiredFields.filter(
      (field) => !billData[field as keyof Bill],
    );
...
async function getBill(db: D1DatabaseClient, billId: string) {
  try {
    // Get bill details
    const billResult = await db.query("SELECT * FROM bills WHERE id = ?", [
      billId,
    ]);
    ...
    if (billData.items && billData.items.length > 0) {
      ...
      try {
        ...
        for (let index = 0; index < billData.items.length; index++) {
          ...
          const srNo = parseInt(
            item.sr_no?.toString() || (index + 1).toString(),
          );
          ...
          if (totalPrice <= 0) {
            throw new Error(
              `Item ${index + 1}: Total price must be greater than 0`,
            );
          }
        ...
        console.error(
          "❌ Failed to insert items, rolling back bill:",
          itemError,
        );
...
async function deleteBill(db: D1DatabaseClient, billId: string) {
  try {
    ...
    const billResult = await db.query("SELECT id FROM bills WHERE id = ?", [
      billId,
    ]);
    const bill = billResult.results?.[0];
